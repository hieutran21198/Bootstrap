...
> Informal quick-reference (outside the seven formal `docs/` tracks). The machine
...
| **architect**              | Design decisions and tradeoffs; produce/validate ADRs & specs; define boundaries; independent review (`writer ≠ reviewer`)                      | Output matches the relevant `docs/` track format, or a review verdict with concrete findings; decisions → `adrs/`, designs → `specs/`, views → `wiki/architecture/`                                  | codegraph (edit docs, no bash)           | —                                      |
