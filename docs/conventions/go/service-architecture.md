...
This workspace is **multi-tenant**: every write MUST declare an RLS scope so [Row-Level Security](../../adrs/0008-tenant-scoped-unit-of-work-rls.md) can enforce isolation at the database. [ADR-0008](../../adrs/0008-tenant-scoped-unit-of-work-rls.md) defines two scopes on one axis — `organization` (bound to one tenant) and `system` (cross-tenant, for background jobs and a future separate back-office service) — both under `NOBYPASSRLS` roles. The portal is tenant-facing (a tenant manages itself), so it uses the `organization` scope exclusively; the `system`-scope entry point is planned for cross-tenant consumers. The port reflects the implemented scope — a single, tenant-scoped entry point and no scope-less auto-commit path.
...
The same scope axis applies to both sides: the `system` scope (cross-tenant, for background jobs and a future separate back-office service — not the tenant-facing portal) will add `system`-scoped entry points (e.g. `DoSystemTransaction` / `DoSystemQuery`) alongside the `organization` ones, binding `app.scope = system` instead of an org id. See [ADR-0008](../../adrs/0008-tenant-scoped-unit-of-work-rls.md) for the two-scope model.
