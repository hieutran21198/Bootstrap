...
- **Delete scratch at close.** When the work item closes, the orchestrator or Scribe verifies that needed evidence, learnings, and durable records have been attached to Linear or re-authored into `docs/`, then deletes `.sdlc/<task-slug>/`. Nothing in `.sdlc/` is the long-term source of truth.
